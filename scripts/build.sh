...
buildIosRelease(){
	...
	if [ "$PRE_RELEASE" = true ] ; then
		...
		./node_modules/.bin/react-native run-ios  --configuration Release --simulator "iPhone 14 Pro"
	fi
}
...
buildIosQA(){
  ...
  echo "BITRISE_GIT_BRANCH: $BITRISE_GIT_BRANCH"
    ...
    echo "$IOS_ENV"
		...
		./node_modules/.bin/react-native run-ios --scheme MetaMask-QA  --configuration Release --simulator "iPhone 14 Pro"
	fi
}
